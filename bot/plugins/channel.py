...
async def connect(bot: Bot, update):
    ...
    if in_db:
        await update.reply_text("Can't You see I'm Here...!!!")
        return
    
    wait_msg = await update.reply_text("ωαιт & ωαт¢н....") #Close the fecking Gro 4a while
    ...
    await wait_msg.edit_text(f"🇾‌🇪‌🇦‌🇭‌ 🇭‌🇴‌🇴‌🇴‌🇴‌... 🇮‌'🇲‌<code>{len(data)}</code> ꫝꫀꪹꫀ..")
...
async def disconnect(bot: Bot, update):
    ...
    wait_msg = await update.reply_text("₩ⱧɎ ₳Ⱡ₩₳Ɏ₴ ₥Ɇ /n 卄爪爪爪... ᗪ乇ㄥ乇ㄒ丨几Ꮆ")
...
async def delall(bot: Bot, update):
    ...
    await update.reply_text("ι'м fє¢кє∂ υρ🤕🥵....")
